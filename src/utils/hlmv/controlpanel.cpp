...
void ControlPanel::SetupBodyWindow(mxTab* pTab)
{
	...
	aMaterialName = new mxListBox(wBody, 500, 25, 540, 160, 0);
	aMaterialName->add("None");
	mxToolTip::add(aMaterialName, "Materials (VMT files) this model has loaded");
	new mxLabel(wBody, 785, 5, 100, 18, "Shader:");
	aShaderName = new mxListBox(wBody, 830, 3, 210, 28, 0);
	aShaderName->add("Select material to show shader");
	return mxToolTip::add(aShaderName, "Shader Used");
}
...
void
ControlPanel::setPolycount(int polycount, int verts, int indexes, int triangles)
{
   #define USE_OLD_CODE 0

   #ifdef USE_OLD_CODE == 0
	int result;
	char v6;
	char Buffer[128];

	//static int savePolycount = -10;
	static int saveVerts = -10;
	static int saveIndexes = -10;
	static int saveTriangles = -10;

	static int savePolycount = -10;
	if (triangles == saveTriangles)
	{
		return;
	}
	saveTriangles = triangles;
	char tmp[128];
	sprintf(tmp, "Polycount: %d", triangles);
	lModelInfo3->setLabel(tmp);

	/*result = triangles;
	if (saveVerts != verts || saveIndexes != indexes || saveTriangles != triangles)
	{
		//return;
		saveTriangles = triangles;
		saveVerts = verts;
		saveIndexes = indexes;
		sprintf(Buffer, "Verts: %d  Indexes: %d  Triangles: %d", verts, indexes, triangles);
		//return setLabel(Buffer, v6);
	}
	lModelInfo3->setLabel(Buffer, "Verts: %d  Indexes: %d  Triangles: %d", verts, indexes, triangles);*/
	//return result;
    #elif USE_NEW_CODE == 1
