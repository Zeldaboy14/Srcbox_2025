...
char g_appTitle[] = "Half-Life 2 Model Viewer v1.22 - Srcbox";
static char recentFiles[8][256] = { "", "", "", "", "", "", "", "" };
extern int g_dxlevel;
bool g_bInError = false;
static ConVar cl_discord_appid("cl_discord_appid", "1383108585671622716", FCVAR_DEVELOPMENTONLY | FCVAR_CHEAT);
...
void MDLViewer::LoadModelFile(const char* pszFile, int slot)
{
	...
	const char* filePathShortened = strrchr(filePath, '\\');

	// if a backslash wasnt found dont...
	if (filePathShortened && V_strlen(filePathShortened) > 1) {
		filePath = filePathShortened + 1;
	}

	if (filePath != NULL)
	{
    sprintf(buffer, "Currently viewing %s", filePath);
    discordPresence.state = "";
    discordPresence.details = buffer;
    discordPresence.startTimestamp = startTimestamp;
    discordPresence.largeImageKey = "srcbox_hl2mv";
...
MDLViewer::handleEvent (mxEvent *event)
{
	...
	case mxEvent::Action:
	{
		...
		case IDC_HELP_ABOUT:
			mxMessageBox (this,
				"Srcbox Model Viewer v0.02a\n"
				"Built upon the Half-Life Model Viewer v2.0 (c) 2006 Valve Corp.\n"
				"Portions (c) 1999 by Mete Ciragan\n\n"
				"Reverse-Engineered code by Zeldaboy14 (c) 2025\n\n"
...
bool CHLModelViewerApp::Create()
{
	...
	if(!pShaderDLL)
	{
		pShaderDLL = "game_shader_dx9.dll";
...
bool CHLModelViewerApp::PreInit()
{
	...
	sprintf(buffer, "No Model Loaded...");
	discordPresence.state = "";
	discordPresence.details = buffer;
	discordPresence.startTimestamp = startTimestamp;
	discordPresence.largeImageKey = "srcbox_hl2mv";
