...
void CHL2GameMovement::TracePlayerBBox( const Vector& start, const Vector& end, unsigned int fMask, int collisionGroup, trace_t& pm )
{
	VPROF( "CHL2GameMovement::TracePlayerBBox" );

#if 0
	Vector traceForward = ( end - start );
	const float traceDist = traceForward.NormalizeInPlace();

	if ( fabsf( traceForward.z ) > 0.98f )
	{
		BaseClass::TracePlayerBBox( start, end, fMask, collisionGroup, pm );
	}

	const Vector playerMins = GetPlayerMins();
	const Vector playerMaxs = GetPlayerMaxs();
	//const Vector playerCenter = ( end + start + playerMins + playerMaxs )/2;
	const Vector playerCenter = end + Vector(0,0,playerMins.z+1);

	//const float playerHeight = playerMaxs.z - playerMins.z;
	const float playerRadius = max( playerMaxs.x, playerMaxs.y ) * 1.414214f; // sqrf( r*r + r*r )
	

	const Vector traceForward2D = Vector( traceForward.x, traceForward.y, 0 ).Normalized();
	const Vector traceRight = CrossProduct( traceForward2D, Vector( 0, 0, 1 ) );

	const int traceCount = 16;
	const float traceStep = (playerRadius*2)/(traceCount-1);
	const Vector traceMins = Vector( -traceStep, -traceStep, traceStep );
	const Vector traceMaxs = Vector( traceStep, traceStep, traceStep );

	trace_t traces[traceCount];
	int hit = -1;

	for ( int i = 0; i < traceCount; i++ )
	{
		const float x = playerRadius - i*traceStep;
		const float y = sqrtf( fabsf( playerRadius*playerRadius - x*x ) );

		const Vector traceStart = playerCenter + traceRight*x + traceForward2D*y; // traceForwardPlanar
		const Vector traceEnd = traceStart + traceForward * traceDist;

		Ray_t ray;
		ray.Init( traceStart, traceEnd, traceMins, traceMaxs );
		UTIL_TraceRay( ray, fMask, mv->m_nPlayerHandle.Get(), collisionGroup, &traces[i] );

		
		if ( traces[i].DidHit() && ( hit < 0 || traces[i].fraction < traces[hit].fraction ) )
		{
			hit = i;
		}
	}
	...
	if ( 1 )
	{
		for ( int i = 0; i < traceCount; i++ )
		{
			int shade = ( traces[i].DidHit() ? 0 : 255 );
			NDebugOverlay::Line( traces[i].startpos, traces[i].endpos, 255,shade,shade, true, 0 );
		}

		NDebugOverlay::Circle( playerCenter, QAngle( 90,0, 0 ), playerRadius, 255,255,0, 50, true, 0 );
		NDebugOverlay::Box( playerCenter, Vector(playerMins.x,playerMins.y,0), Vector(playerMaxs.x,playerMaxs.y,0), 255,255,0,50, 0 );
	}
	...
	{
		const Vector offset = traces[hit].endpos - traces[hit].startpos;

		pm = traces[hit];
		pm.startpos = start;
		pm.endpos = start + offset - offset.Normalized()*2;
	}
#else
	BaseClass::TracePlayerBBox( start, end, fMask, collisionGroup, pm );
#endif
