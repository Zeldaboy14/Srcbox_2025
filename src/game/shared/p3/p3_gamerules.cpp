...
//
// Purpose: The Postl 3 game rules, such as the relationship tables and ammo
//			damage cvars.
//
//=============================================================================

#include "cbase.h"
#include "p3_gamerules.h"
#include "ammodef.h"
...
REGISTER_GAMERULES_CLASS( CPostal3 );

BEGIN_NETWORK_TABLE_NOBASE( CPostal3, DT_P3GameRules )
	#ifdef CLIENT_DLL
...
LINK_ENTITY_TO_CLASS( p3_gamerules, CPostal3Proxy );
IMPLEMENT_NETWORKCLASS_ALIASED( Postal3Proxy, DT_Postal3Proxy )


#ifdef CLIENT_DLL
	void RecvProxy_P3GameRules( const RecvProp *pProp, void **pOut, void *pData, int objectID )
	{
		CPostal3 *pRules = P3GameRules();
		Assert( pRules );
		*pOut = pRules;
	}

	BEGIN_RECV_TABLE( CPostal3Proxy, DT_Postal3Proxy )
		RecvPropDataTable( "p3_gamerules_data", 0, 0, &REFERENCE_RECV_TABLE( DT_P3GameRules ), RecvProxy_P3GameRules )
	END_RECV_TABLE()
#else
	void* SendProxy_P3GameRules( const SendProp *pProp, const void *pStructBase, const void *pData, CSendProxyRecipients *pRecipients, int objectID )
	{
		CPostal3 *pRules = P3GameRules();
		Assert( pRules );
	...
	BEGIN_SEND_TABLE( CPostal3Proxy, DT_Postal3Proxy )
		SendPropDataTable( "p3_gamerules_data", 0, &REFERENCE_SEND_TABLE( DT_P3GameRules ), SendProxy_P3GameRules )
	END_SEND_TABLE()
#endif
...
//-----------------------------------------------------------------------------
int CPostal3::Damage_GetTimeBased( void )
{
#ifdef HL2_EPISODIC
...
// Output :		bool
//-----------------------------------------------------------------------------
bool CPostal3::Damage_IsTimeBased( int iDmgType )
{
...
ConVar  alyx_darkness_force( "alyx_darkness_force", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
	...
	//-----------------------------------------------------------------------------
	bool CPostal3::ClientCommand( CBaseEntity *pEdict, const CCommand &args )
	{
	...
	//-----------------------------------------------------------------------------
	void CPostal3::PlayerSpawn( CBasePlayer *pPlayer )
	{
		pPlayer->EquipSuit( false );

		/*
		pPlayer->GiveAmmo( INT_MAX, GetAmmoDef()->Index( "DesertEagle" ), true );
		pPlayer->GiveAmmo( 100, GetAmmoDef()->Index( "BadgerPower" ), true );
		pPlayer->GiveAmmo( 200, GetAmmoDef()->Index( "Taser" ), true );
		pPlayer->GiveAmmo( INT_MAX, GetAmmoDef()->Index( "M60" ), true );
		pPlayer->GiveAmmo( INT_MAX, GetAmmoDef()->Index( "M16" ), true );
		pPlayer->GiveAmmo( 30, GetAmmoDef()->Index( "Molotov" ), true );
		pPlayer->GiveAmmo( 30, GetAmmoDef()->Index( "Beehive" ), true );
		pPlayer->GiveAmmo( 30, GetAmmoDef()->Index( "Grenade" ), true );
		pPlayer->GiveAmmo( 100, GetAmmoDef()->Index( "CatnipSpray" ), true );
		//pPlayer->GiveAmmo( 100, GetAmmoDef()->Index( "CatnipCan" ), true );
		//pPlayer->GiveAmmo( 100, GetAmmoDef()->Index( "PepperSprayCan" ), true );
		pPlayer->GiveAmmo( 50, GetAmmoDef()->Index( "Buckshot" ), true );
		*/

		//pPlayer->GiveAmmo( 100, GetAmmoDef()->Index( "Cat" ), true );
		pPlayer->GiveAmmo( 100, GetAmmoDef()->Index( "Motorhead" ), true );

		// даем предметы
		{
			const char* p = P3_Registry_GetString( "player_items" );
			char token[128];

			while ( (p = nexttoken( token, p, ' ' )) != 0 )
			{
				pPlayer->GiveNamedItem( token );
			}

			// выбирается это оружие, т.к. добавляется последним
			pPlayer->GiveNamedItem( "p3_weapon_emptyhands" );
			pPlayer->GiveNamedItem( "p3_weapon_weemote" );
		}
	}
	...
	// Output  :
	//------------------------------------------------------------------------------
	void CPostal3::InitDefaultAIRelationships( void )
	{
		int i, j;
	...
	// Output  :
	//------------------------------------------------------------------------------
	const char* CPostal3::AIClassText(int classType)
	{
		switch (classType)
		{
	...
	void CPostal3::PlayerThink( CBasePlayer *pPlayer )
	{
	}

	void CPostal3::Think( void )
	{
		BaseClass::Think();
	...
	//-----------------------------------------------------------------------------
	float CPostal3::GetAmmoDamage( CBaseEntity *pAttacker, CBaseEntity *pVictim, int nAmmoType )
	{
		float flDamage = 0.0f;
	...
	//-----------------------------------------------------------------------------
	//-----------------------------------------------------------------------------
	bool CPostal3::AllowDamage( CBaseEntity *pVictim, const CTakeDamageInfo &info )
	{
#ifndef CLIENT_DLL
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool CPostal3::NPC_ShouldDropHealth( CBasePlayer *pRecipient )
	{
	...
	// Output : Returns true on success, false on failure.
	//-----------------------------------------------------------------------------
	bool CPostal3::NPC_ShouldDropGrenade( CBasePlayer *pRecipient )
	{
		// Can only do this every so often
	...
	//-----------------------------------------------------------------------------
	void CPostal3::NPC_DroppedHealth( void )
	{
	...
	//-----------------------------------------------------------------------------
	void CPostal3::NPC_DroppedGrenade( void )
	{
...
// ------------------------------------------------------------------------------------ //
// Shared CPostal3 implementation.
// ------------------------------------------------------------------------------------ //
bool CPostal3::ShouldCollide( int collisionGroup0, int collisionGroup1 )
{
	...
	#else
	if ( collisionGroup1 == COLLISION_GROUP_NPC_ACTOR && collisionGroup0 != COLLISION_GROUP_PLAYER && collisionGroup0 != COLLISION_GROUP_VEHICLE && collisionGroup0 != COLLISION_GROUP_IN_VEHICLE )
	{
		collisionGroup1 = COLLISION_GROUP_NPC;
	}
	#endif
	...
	if( collisionGroup1 == P3COLLISION_GROUP_PROHIBITED_AREA && collisionGroup0 != P3COLLISION_GROUP_TEST_PROHIBITED_AREA ) 
		return false;
...
//---------------------------------------------------------
//---------------------------------------------------------
void CPostal3::AdjustPlayerDamageTaken( CTakeDamageInfo *pInfo )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
float CPostal3::AdjustPlayerDamageInflicted( float damage )
{
	switch( GetSkillLevel() ) 
	{
	case SKILL_EASY:
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CPostal3::ShouldUseRobustRadiusDamage(CBaseEntity *pEntity)
{
#ifdef CLIENT_DLL
	return false;
#endif
...
//---------------------------------------------------------
//---------------------------------------------------------
bool CPostal3::ShouldAutoAim( CBasePlayer *pPlayer, edict_t *target )
{
...
//---------------------------------------------------------
//---------------------------------------------------------
float CPostal3::GetAutoAimScale( CBasePlayer *pPlayer )
{
#ifdef _X360
	return 1.0f;
#else
...
//---------------------------------------------------------
//---------------------------------------------------------
float CPostal3::GetAmmoQuantityScale( int iAmmoIndex )
{
	switch( GetSkillLevel() )
	{
	case SKILL_EASY:
...
void CPostal3::LevelInitPreEntity()
{
...
void
CPostal3::DeathNoticePlayer( CP3_Player *pVictim, const CTakeDamageInfo &info )
{
	IGameEvent * event = gameeventmanager->CreateEvent( "player_death", false );
	...
	if ( event )
	{
		...
		event->SetInt( "victim_id", pVictim->entindex() );
...
void
CPostal3::DeathNoticeNPC( CP3_NPC *pVictim, const CTakeDamageInfo &info )
{
	IGameEvent * event = gameeventmanager->CreateEvent( "npc_death", false );

	if ( event )
	{
		CBasePlayer *p = ToP3Player( info.GetAttacker() );

		event->SetInt( "attacker_id", info.GetAttacker() ? info.GetAttacker()->entindex() : 0 );
		event->SetInt( "userid", p ? p->GetUserID() : -1 );
		...
		{
			if ( info.GetInflictor() && info.GetInflictor()->IsCat() )
			{
				event->SetInt( "weapon_id", P3_WEAPON_CAT );
			}
			else if ( info.GetInflictor() && info.GetInflictor()->IsDog() )
			{
				event->SetInt( "weapon_id", P3_WEAPON_DOG);
			}
			else
			{
				event->SetInt( "weapon_id", info.GetWeapon() ? info.GetWeapon()->MyP3BaseWeapon()->GetWeaponID() : 0 );
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("DesertEagle",		DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_p3_deserteagle",	"sk_npc_dmg_p3_deserteagle",	"sk_max_p3_deserteagle",	"Ammo.Pistol",	BULLET_IMPULSE(200, 1225), 0 );
		def.AddAmmoType("M16",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_p3_m16",			"sk_npc_dmg_p3_m16",			"sk_max_p3_m16",			"Ammo.M16",		BULLET_IMPULSE(200, 1225), 0 );
		def.AddAmmoType("M136",				DMG_BLAST,					TRACER_NONE,			"sk_plr_dmg_p3_m136",			"sk_npc_dmg_p3_m136",			"sk_max_p3_m136",			"Ammo.M136",	0, 0 );
		def.AddAmmoType("M60",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_p3_m60",			"sk_npc_dmg_p3_m60",			"sk_max_p3_m60",			"Ammo.M60",		BULLET_IMPULSE(400, 2450), 0 );
		def.AddAmmoType("Taser",			DMG_PARALYZE,				TRACER_NONE,			"sk_plr_dmg_p3_taser",			"sk_npc_dmg_p3_taser",			"sk_max_p3_taser",			"Ammo.Taser",	0.01f, 0 );
		def.AddAmmoType("Cat",				DMG_BURN,					TRACER_NONE,			0,								0,								10,							"Ammo.Cat",		0, 0 );
		def.AddAmmoType("Molotov",			DMG_BURN,					TRACER_NONE,			"sk_plr_dmg_p3_molotov",		"sk_npc_dmg_p3_molotov",		"sk_max_p3_molotov",		"Ammo.Molotov",	0, 0);
		def.AddAmmoType("Beehive",			DMG_GENERIC,				TRACER_NONE,			0,								0,								10,							"Ammo.Beehive",	0, 0);
		def.AddAmmoType("CatnipCan",		DMG_CLUB,					TRACER_NONE,			0,								0,								6,						    "Ammo.Can",0, 0);
		def.AddAmmoType("CatnipSpray",		DMG_NERVEGAS,				TRACER_NONE,			0,								0,								99,							"Ammo.Catnip",	0, 0);
		def.AddAmmoType("Seed",				DMG_NERVEGAS,				TRACER_NONE,			0,								0,								99,							"Ammo.Seed",	0, 0);
		def.AddAmmoType("PepperSprayCan",	DMG_CLUB,					TRACER_NONE,			0,								0,								6,						    "Ammo.Can",0, 0);
		def.AddAmmoType("PepperSprayGas",	DMG_BURN,					TRACER_NONE,			0,								0,								100,						"Ammo.Gasoline",0, 0);
		def.AddAmmoType("Gasoline",			DMG_BURN,					TRACER_NONE,			0,								0,								100,						"Ammo.Gasoline",0, 0);
		def.AddAmmoType("Buckshot",			DMG_BULLET | DMG_BUCKSHOT,	TRACER_LINE,			"sk_plr_dmg_p3_buckshot",		"sk_npc_dmg_p3_buckshot",		"sk_max_p3_buckshot",		"Ammo.Buckshot",BULLET_IMPULSE(400, 1200), 0 );
		def.AddAmmoType("Grenade",			DMG_BURN,					TRACER_NONE,			"sk_plr_dmg_p3_grenade",		"sk_npc_dmg_p3_grenade",		"sk_max_p3_grenade",		"Ammo.Grenade",	0, 0);
		def.AddAmmoType("CrotchyGrenade",	DMG_KROTCHY,				TRACER_NONE,			"sk_plr_dmg_p3_crotchy_grenade","sk_npc_dmg_p3_crotchy_grenade","sk_max_p3_crotchy_grenade","Ammo.KGrenade",0, 0);
		def.AddAmmoType("LaserPen",			DMG_ENERGYBEAM,				TRACER_BEAM,			0,								0,								"sk_max_p3_laserpen",		"Ammo.Laserpen",0, 0);
		def.AddAmmoType("BadgerPower",		DMG_GENERIC,				TRACER_BEAM,			0,								0,								0,							"Ammo.Badger",	0, 0);
		def.AddAmmoType("Motorhead",		DMG_GENERIC,				TRACER_BEAM,			0,								0,								1,							"Ammo.Mhead",	0, 0);
		def.AddAmmoType("Glock17",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_p3_glock17",		"sk_npc_dmg_p3_glock17",		"sk_max_p3_glock17",		"Ammo.Glock17",	BULLET_IMPULSE(200, 1225), 0 );
		def.AddAmmoType("P90",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	"sk_plr_dmg_p3_p90",			"sk_npc_dmg_p3_p90",			"sk_max_p3_p90",			"Ammo.P90",		BULLET_IMPULSE(400, 2450), 0 );
	}
