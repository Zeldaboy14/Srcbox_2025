...
#include "cbase.h"
#include "simulators\p3_simulator_viewblend_shared.h"
...
BEGIN_SIMPLE_DATADESC( P3_SimulatorViewSmoothingData_t )
	...
	DEFINE_FIELD( flEnterExitStartTime, FIELD_TIME ),
...
// end < t :			f(t) = t
float RemapAngleRange( float startInterval, float endInterval, float value, P3_SimulatorRemapAngleRange_CurvePart_t *peCurvePart )
{
	// Fixup the roll
	...
	if ( absAngle < startInterval )
	{
		if ( peCurvePart )
		{
			*peCurvePart = P3_SimulatorRemapAngleRange_CurvePart_Zero;
		}
		value = 0;
	}
	// in spline range?
	else if ( absAngle <= endInterval )
	{
		...
		if ( peCurvePart )
		{
			*peCurvePart = P3_SimulatorRemapAngleRange_CurvePart_Spline;
		}
		value = newAngle;
	}
	...
	{
		*peCurvePart = P3_SimulatorRemapAngleRange_CurvePart_Linear;
	}
...
//-----------------------------------------------------------------------------
float ApplyViewLocking( float flAngleRaw, float flAngleClamped, P3_SimulatorViewLockData_t &lockData, P3_SimulatorRemapAngleRange_CurvePart_t eCurvePart )
{
	...
	if ( ( eCurvePart == P3_SimulatorRemapAngleRange_CurvePart_Linear ) ||
		( lockData.bLocked && ( eCurvePart == P3_SimulatorRemapAngleRange_CurvePart_Spline ) ) )
	{
		//Msg( "LOCKED\n" );
...
//			vehicleEyeAngles - 
//-----------------------------------------------------------------------------
void RemapViewAngles( P3_SimulatorViewSmoothingData_t *pData, QAngle &vehicleEyeAngles )
{
	QAngle vecEyeAnglesRemapped;

	// Clamp pitch.
	P3_SimulatorRemapAngleRange_CurvePart_t ePitchCurvePart;
	...
	P3_SimulatorRemapAngleRange_CurvePart_t eRollCurvePart;
...
{	
	...
	*pAbsAngles = pPlayer->EyeAngles();
	...
	{
		CP3_Simulator *pDriveable = assert_cast<CP3_Simulator*>(pData->pVehicle);
