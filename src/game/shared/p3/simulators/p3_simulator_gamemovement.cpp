...
//=============================================================================//
#include "cbase.h"
#include "p3_simulator_gamemovement.h"
...
#include "p3/NPC/p3_npc.h"

#include "vphysics/friction.h"
#include "p3/utils/p3_collisionblocks.h"

#undef STICK_EXTENTS
#define STICK_EXTENTS	450.0f

#define MOVEMENT_COLLISION_GROUP	COLLISION_GROUP_VEHICLE

extern ConVar p3_simulator_maxspeed;

ConVar p3_npc_simulator_prediction_hit_debug( "p3_npc_simulator_prediction_hit_debug", "0" );

#ifdef _XBOX
	ConVar	p3_simulator_sv_accelerate	( "p3_simulator_sv_accelerate", "3", FCVAR_NOTIFY | FCVAR_REPLICATED);
#else
	ConVar	p3_simulator_sv_accelerate	( "p3_simulator_sv_accelerate", "4", FCVAR_NOTIFY | FCVAR_REPLICATED/* | FCVAR_DEVELOPMENTONLY*/);
#endif//_XBOX
...
extern IFileSystem *filesystem;
	...
	static ConVarRef dispcoll_drawplane( "dispcoll_drawplane" );
#endif
...
#if defined( HL2_DLL )
static ConVarRef xc_uncrouch_on_jump( "xc_uncrouch_on_jump" );
#endif

}

ConVar p3_simulator_debug_collisions( "p3_simulator_debug_collisions", "0" );
...
extern ConVar option_duck_method;
...
void DiffPrint( bool bServer, int nCommandNumber, char const *fmt, ... )
{
	...
	CP3_Simulator *pPlayer = CBaseEntity::GetPredictionPlayer();
...
void CP3_SimulatorGameMovement::DiffPrint( char const *fmt, ... )
{
	if ( !player )
		return;
...
inline void DrawTraceCollision( trace_t& trace )
{
	if( p3_simulator_debug_collisions.GetBool() && trace.fraction < 1 )
	{
		const Vector& collisionPt = trace.collisionPoint;
		NDebugOverlay::Line( collisionPt, collisionPt+trace.plane.normal*10, 255,255,0, false, 0.5f );
	}
}
...
//-----------------------------------------------------------------------------
CP3_SimulatorGameMovement::CP3_SimulatorGameMovement( void )
{
	...
	m_nMovementCollisionGroup = MOVEMENT_COLLISION_GROUP;

	m_vecHullMins = VEC_HULL_MIN;
	m_vecHullMaxs = VEC_HULL_MAX;
}
...
//-----------------------------------------------------------------------------
unsigned int CP3_SimulatorGameMovement::PlayerSolidMask( bool brushOnly ) const
{
	if ( player && player->GetPassenger( VEHICLE_ROLE_DRIVER ) && player->GetPassenger( VEHICLE_ROLE_DRIVER )->IsNPC() )
	{
		return ( brushOnly ) ? MASK_NPCSOLID_BRUSHONLY : MASK_NPCSOLID;
	}
	else
	{
...
// Output : int
//-----------------------------------------------------------------------------
int CP3_SimulatorGameMovement::GetCheckInterval( IntervalType_t type )
{
	int tickInterval = 1;
	switch ( type )
	{
...
//-----------------------------------------------------------------------------
bool CP3_SimulatorGameMovement::CheckInterval( IntervalType_t type )
{
...
//const Vector VEC_SIM_HULL_MIN( -25, -25, 0 );
//const Vector VEC_SIM_HULL_MAX( 25, 25, 75 );
//
//const Vector VEC_SIM_DUCK_HULL_MIN( VEC_SIM_HULL_MIN );
//const Vector VEC_SIM_DUCK_HULL_MAX( VEC_SIM_HULL_MAX );
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CP3_SimulatorGameMovement::GetPlayerMins( bool ducked ) const
{
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CP3_SimulatorGameMovement::GetPlayerMaxs( bool ducked ) const
{	
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CP3_SimulatorGameMovement::GetPlayerMins( void ) const
{
	return m_vecHullMins; //player->GetPlayerMins();
/*
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CP3_SimulatorGameMovement::GetPlayerMaxs( void ) const
{	
	return m_vecHullMaxs; //player->GetPlayerMaxs();
/*
	if ( player->IsObserver() )
	{
...
// Output : const Vector
//-----------------------------------------------------------------------------
const Vector& CP3_SimulatorGameMovement::GetPlayerViewOffset( bool ducked ) const
{
...
//-----------------------------------------------------------------------------
inline void CP3_SimulatorGameMovement::TracePlayerBBox( const Vector& start, const Vector& end, unsigned int fMask, int collisionGroup, trace_t& pm )
{
	AI_PROFILE_SCOPE( "CP3_SimulatorGameMovement::TracePlayerBBox" );
...
CBaseHandle CP3_SimulatorGameMovement::TestPlayerPosition( const Vector& pos, int collisionGroup, trace_t& pm )
{
	Ray_t ray;
	ray.Init( pos, pos, GetPlayerMins(), GetPlayerMaxs(), p3_simulator_use_sphere.GetBool() );
	UTIL_TraceRay( ray, PlayerSolidMask(), mv->m_nPlayerHandle.Get(), collisionGroup, &pm );
	if ( (pm.contents & PlayerSolidMask()) && pm.m_pEnt && pm.DidHitNonWorldEntity() )
	{
...
bool CP3_SimulatorGameMovement::IsWet() const
{
...
void CP3_SimulatorGameMovement::PlantFootprint( surfacedata_t *psurface )
{
...
void CP3_Simulator::UpdateWetness()
{
	// BRJ 1/7/01
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::CategorizeGroundSurface( trace_t &pm )
{
	IPhysicsSurfaceProps *physprops = m_MoveHelper.GetSurfaceProps();
...
bool CP3_SimulatorGameMovement::IsDead( void ) const
{
	return false;
...
//-----------------------------------------------------------------------------
float CP3_SimulatorGameMovement::ComputeConstraintSpeedFactor( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::CheckParameters( void )
{
	QAngle	v_angle;
...
void CP3_SimulatorGameMovement::ReduceTimers( void )
{
...
// Input  : *pMove - 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::ProcessMovement( CP3_Simulator *pPlayer, CP3_SimulatorMoveData *pMove )
{
	...
	mv = pMove;
	mv->m_flMaxSpeed = p3_simulator_maxspeed.GetFloat();
	
	m_nMovementState = StateMoving;
	m_LastTrace.m_pEnt = NULL;
	m_LastTrace.fraction = 1.f;
...
void CP3_SimulatorGameMovement::StartTrackPredictionErrors( CP3_Simulator *pPlayer )
{
	player = pPlayer;
...
void CP3_SimulatorGameMovement::FinishTrackPredictionErrors( CP3_Simulator *pPlayer )
{
#if PREDICTION_ERROR_CHECK_LEVEL > 0
	Assert( player == pPlayer );
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FinishMove( void )
{
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::DecayPunchAngle( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::StartGravity( void )
{
	float ent_gravity;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::CheckWaterJump( void )
{
	Vector	flatforward;
	Vector forward;
	Vector	flatvelocity;
	...
	trace_t tr;
	TracePlayerPrimitive( vecStart, vecEnd, PlayerSolidMask(), m_nMovementCollisionGroup, tr );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::WaterJump( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::WaterMove( void )
{
	int		i;
	Vector	wishvel;
	float	wishspeed;
	...
	if (wishspeed >= 0.1f)  // old !
	{
		...
		if (addspeed > 0)
		{
			...
			accelspeed = p3_simulator_sv_accelerate.GetFloat() * wishspeed * gpGlobals->frametime * player->m_surfaceFriction;
	...
	TracePlayerPrimitive( mv->GetAbsOrigin(), dest, PlayerSolidMask(), m_nMovementCollisionGroup, pm );
	if ( pm.fraction == 1.0f )
	{
		...
		TracePlayerPrimitive( start, dest, PlayerSolidMask(), m_nMovementCollisionGroup, pm );
...
void CP3_SimulatorGameMovement::AddDebugValue( int val )
{
	if ( player->m_iDebug[CP3_Simulator::DEBUG_ARRAY_SIZE-1]!=val )
	{
		for ( int i=0; i<CP3_Simulator::DEBUG_ARRAY_SIZE-1; ++i )
		{
			player->m_iDebug[i]=player->m_iDebug[i+1];
		}
		player->m_iDebug[CP3_Simulator::DEBUG_ARRAY_SIZE-1]=val;
	}
}
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::StepMove( Vector &vecDestination, trace_t &trace )
{	
	if ( mv->GetAbsOrigin().z - player->GetAbsOrigin().z > player->GetStepSize() )
	{	
		//if ( trace.m_pEnt && trace.plane.normal.z<0.5f )
		{
			m_nMovementState = StateObstacle;		
	...
	first_trace=trace;
	// Slide move down.
	int down_blocked = TryPlayerMove( &vecEndPos, &trace );
	...
	TracePlayerPrimitive( mv->GetAbsOrigin(), vecEndPos, PlayerSolidMask(), m_nMovementCollisionGroup, trace );
	if ( !trace.startsolid && !trace.allsolid )
	{	
		mv->SetAbsOrigin( trace.endpos );
	}

	P3_Simulator::DrawTraceCollision( trace );

	// Slide move up.
	int up_blocked = TryPlayerMove();
	...
	if ( down_blocked && up_blocked && (trace.m_pEnt || first_trace.m_pEnt) )
	{
		m_nMovementState = StateObstacle;		
		m_LastTrace = first_trace.m_pEnt ? first_trace : trace;		
	}
	...
	//B2S добавлена проверка trace.m_pEnt
	if ( !trace.plane.normal.IsZero() && up_blocked && trace.m_pEnt && trace.plane.normal[2] < 0.7 )
	{	
		AddDebugValue( 3 );
...
ESimulatorMovementState	CP3_SimulatorGameMovement::MoveProbe( Vector &vecDestination, trace_t &trace ) const
{
	ESimulatorMovementState movement_result = StateInvalid;
	// Try sliding forward both on ground and up 16 pixels
	//  take the move that goes farthest
	Vector vecPos, vecVel;
	VectorCopy( mv->GetAbsOrigin(), vecPos );
	...
	first_trace=trace;
	// Slide move down.
	int down_blocked = ProbePlayerMove( &vecEndPos, &trace );
	...
	// Move up a stair height.
	VectorCopy( vecPos, vecEndPos );
	if ( player->m_Local.m_bAllowAutoMovement )
	{
		vecEndPos.z += player->m_Local.m_flStepSize + DIST_EPSILON;
	}
	
	TracePlayerPrimitive( vecPos, vecEndPos, PlayerSolidMask(), m_nMovementCollisionGroup, trace );
	if ( !trace.startsolid && !trace.allsolid )
	{
		//mv->SetAbsOrigin( trace.endpos );
	}
	...
	// Slide move up.
	int up_blocked = ProbePlayerMove();
	...
	if ( down_blocked && up_blocked && (trace.m_pEnt || first_trace.m_pEnt) )
	{
		movement_result = StateObstacle;		
		trace = first_trace.m_pEnt ? first_trace : trace;
	}
	...
	// If we are not on the ground any more then use the original movement attempt.
	//B2S добавлена проверка trace.m_pEnt
	if ( up_blocked && trace.m_pEnt && trace.plane.normal[2] < 0.7 )
	{
		/*mv->SetAbsOrigin( vecDownPos );
		VectorCopy( vecDownVel, mv->m_vecVelocity );
		float flStepDist = mv->GetAbsOrigin().z - vecPos.z;
	...
	float flStepDist = vecPos.z - vecPos.z;
	if ( flStepDist > 0 )
	{
		//mv->m_outStepHeight += flStepDist;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::Friction( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FinishGravity( void )
{
	float ent_gravity;
...
//			accel - 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::AirAccelerate( Vector& wishdir, float wishspeed, float accel )
{
	int i;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::AirMove( void )
{
	int			i;
	Vector		wishvel;
	float		fmove, smove;
...
bool CP3_SimulatorGameMovement::CanAccelerate()
{
...
//			accel - 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::Accelerate( Vector& wishdir, float wishspeed, float accel )
{
	int i;
	float addspeed, accelspeed, currentspeed;
	...
	for (i=0 ; i<3 ; i++)
	{
		mv->m_vecVelocity[i] = currentspeed*wishdir[i] + accelspeed * wishdir[i];	
	}
}

bool IsTraceValid(trace_t &tr)
{
	return !( (tr.surface.flags & SURF_NODRAW) && (tr.m_pEnt && (tr.m_pEnt->GetEffects() & EF_NODRAW) ) );
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::WalkMove( void )
{
	int i;
	...
	VectorCopy (wishvel, mv->m_vecWishDirection);
	...
	float move = fmove + smove;
	if ( move > STICK_EXTENTS )
		move = STICK_EXTENTS;
	else
		if ( move < -STICK_EXTENTS )
			move = -STICK_EXTENTS;
	...
	if ( player->MovementOptimizationsEnabled() )
		wishspeed *= 0.6f;
	
	// Set pmove velocity
	mv->m_vecVelocity[2] = 0;
	Accelerate ( wishdir, wishspeed, p3_simulator_sv_accelerate.GetFloat() );	
	mv->m_vecVelocity[2] = 0;
	...
	if ( wishspeed>0 )
	{
		for ( int i=0; i<3 && pm.m_pEnt && pm.m_pEnt->IsNPC() ;++i )
		{
			CP3_NPC *pNPC = pm.m_pEnt->MyP3NPCPointer();
			if ( pNPC && NULL==pNPC->GetParent() && pNPC->GetRenderMode()!=kRenderNone )
			{
				CTakeDamageInfo dmgInfo( player, player, 0, DMG_FALL );
				pNPC->HandleDamageLikeRagdoll( dmgInfo, false );
			}			

			TracePlayerPrimitive( mv->GetAbsOrigin(), dest, PlayerSolidMask(), COLLISION_GROUP_PLAYER_MOVEMENT, pm );
		}
	}

	//NDebugOverlay::Line( mv->GetAbsOrigin(), dest, 255, 255, 255, false, 5.0f );
	...
	{
		trace_t trace;
		//TracePlayerPrimitive( mv->GetAbsOrigin(), mv->GetAbsOrigin(), PlayerSolidMask(), m_nMovementCollisionGroup, trace );
		//UTIL_TraceHull( mv->GetAbsOrigin(), mv->GetAbsOrigin(), GetPlayerMins(), GetPlayerMaxs(), MASK_PLAYERSOLID_BRUSHONLY, player, COLLISION_GROUP_PLAYER_MOVEMENT, &trace );
		//if ( trace.m_pEnt )
		...
		/*if ( CheckStuck() )
		{
			NDebugOverlay::Box( trace.endpos, -Vector(5,5,5), Vector(5,5,5), 255,120,120, true, 15000);			
		}*/

		return;
	}
	
	P3_Simulator::DrawTraceCollision( pm );
	...
	if ( pm.m_pEnt && pm.m_pEnt!=player && pm.m_pEnt->IsMovementObstacleForEntity(player) )         
	{			
		m_nMovementState = StateObstacle;
		m_LastTrace = pm;
		
		m_MoveHelper.AddToTouched( pm, mv->m_vecVelocity );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FullWalkMove( )
{
	if ( !CheckWater() ) 
	{
		StartGravity();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FullObserverMove( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FullNoClipMove( float factor, float maxacceleration )
{
	Vector wishvel;
	Vector forward, right, up;
	Vector wishdir;
	float wishspeed;
	float maxspeed = p3_simulator_maxspeed.GetFloat() * factor;
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::PlaySwimSound()
{
	m_MoveHelper.StartSound( mv->GetAbsOrigin(), "Player.Swim" );
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CP3_SimulatorGameMovement::CheckJumpButton( void )
{
	//if (player->pl.deadflag)
	//{
	//	mv->m_nOldButtons |= IN_JUMP ;	// don't jump again until released
	...
	m_MoveHelper.PlayerSetAnimation( PLAYER_JUMP );
...
//#if defined( HL2_DLL )
//
//	if ( P3_Simulator::xc_uncrouch_on_jump.GetBool() )
//	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FullLadderMove()
{
	CheckWater();
...
// Output : int
//-----------------------------------------------------------------------------
int CP3_SimulatorGameMovement::TryPlayerMove( Vector *pFirstDest, trace_t *pFirstTrace, trace_t *pOutStuck )
{
	...
	for (bumpcount=0 ; bumpcount < numbumps; bumpcount++)
	{
		...
		if ( g_bMovementOptimizations )
		{
			...
			else
			{
				...
				trace_t foo;
				TracePlayerPrimitive( mv->GetAbsOrigin(), mv->GetAbsOrigin(), PlayerSolidMask(), m_nMovementCollisionGroup, foo );
				if ( foo.startsolid || foo.fraction != 1.0f )
				{
					Msg( "bah\n" );
				}
#endif
				TracePlayerPrimitive( mv->GetAbsOrigin(), end, PlayerSolidMask(), m_nMovementCollisionGroup, pm );
			}
		}
		else
		{
		...
		{			
			...
			if ( numbumps > 0 && pm.fraction == 1 )
			{
				...
				trace_t stuck;
				TracePlayerPrimitive( pm.endpos, pm.endpos, PlayerSolidMask(), m_nMovementCollisionGroup, stuck );

				P3_Simulator::DrawTraceCollision( stuck );
...
#if defined( PLAYER_GETTING_STUCK_TESTING )
			trace_t foo;
			TracePlayerPrimitive( pm.endpos, pm.endpos, PlayerSolidMask(), m_nMovementCollisionGroup, foo );
			if ( foo.startsolid || foo.fraction != 1.0f )
			{
				Msg( "Player will become stuck!!!\n" );
			}
		...
		//  step or wall
		if( fabs( pm.plane.normal[2] ) < 0.01f )
		{
			blocked |= 2;		// step / wall
		}
...
// Output : int
//-----------------------------------------------------------------------------
int CP3_SimulatorGameMovement::ProbePlayerMove( Vector *pFirstDest, trace_t *pFirstTrace, trace_t *pOutStuck ) const
{
	int			bumpcount, numbumps;
	Vector		dir;
	float		d;
...
//-----------------------------------------------------------------------------
inline bool CP3_SimulatorGameMovement::OnLadder( trace_t &trace )
{
	if ( trace.contents & CONTENTS_LADDER )
		return true;

	IPhysicsSurfaceProps *pPhysProps = m_MoveHelper.GetSurfaceProps();
	if ( pPhysProps )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CP3_SimulatorGameMovement::LadderMove( void )
{
	trace_t pm;
	bool onFloor;
	Vector floor;
	...
	TracePlayerPrimitive( mv->GetAbsOrigin(), end, LadderMask(), m_nMovementCollisionGroup, pm );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::CheckVelocity( void )
{
	int i;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::AddGravity( void )
{
	float ent_gravity;
...
// Output : trace_t
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::PushEntity( Vector& push, trace_t *pTrace )
{
	Vector	end;
		
	VectorAdd (mv->GetAbsOrigin(), push, end);
	TracePlayerPrimitive( mv->GetAbsOrigin(), end, PlayerSolidMask(), m_nMovementCollisionGroup, *pTrace );
	...
	{
		m_MoveHelper.AddToTouched( *pTrace, mv->m_vecVelocity );
	}
}	
...
// Output : int
//-----------------------------------------------------------------------------
int CP3_SimulatorGameMovement::ClipVelocity( Vector& in, Vector& normal, Vector& out, float overbounce ) const
{
	float	backoff;
	float	change;
	float angle;
	...
	if (angle > 0)			// If the plane that is blocking us has a positive z component, then assume it's a floor.
		blocked |= 0x01;	// 
	if ( fabs(angle) < 0.01f )				// If the plane has no Z, it is vertical (wall/step)
		blocked |= 0x02;	// 
...
// Output : float 
//-----------------------------------------------------------------------------
float CP3_SimulatorGameMovement::CalcRoll ( const QAngle &angles, const Vector &velocity, float rollangle, float rollspeed )
{
	float   sign;
	float   side;
	float   value;
...
void CreateStuckTable( void )
{
	...
	memset(P3_Simulator::rgv3tStuckTable, 0, sizeof(P3_Simulator::rgv3tStuckTable));
	...
	{
		P3_Simulator::rgv3tStuckTable[idx][0] = x;
		P3_Simulator::rgv3tStuckTable[idx][1] = y;
		P3_Simulator::rgv3tStuckTable[idx][2] = z;
		idx++;
	}
	x = z = 0;
	// Y moves
	...
	Assert( idx < sizeof(P3_Simulator::rgv3tStuckTable)/sizeof(P3_Simulator::rgv3tStuckTable[0]));
}
#else
...
// Output : int
//-----------------------------------------------------------------------------
int GetRandomStuckOffsets( CP3_Simulator *pPlayer, Vector& offset)
{
 // Last time we did a full
	int idx;
	idx = pPlayer->m_StuckLast++;

	VectorCopy(P3_Simulator::rgv3tStuckTable[idx % 54], offset);
...
//			server - 
//-----------------------------------------------------------------------------
void ResetStuckOffsets( CP3_Simulator *pPlayer )
{
...
// Output : int
//-----------------------------------------------------------------------------
int CP3_SimulatorGameMovement::CheckStuck( void )
{
	Vector base;
	Vector offset;
	Vector test;
	...
	P3_Simulator::CreateStuckTable();

	if ( !mv )
		return 0;

	hitent = TestPlayerPosition( mv->GetAbsOrigin(), m_nMovementCollisionGroup, traceresult );
	if ( hitent == INVALID_ENTITY_HANDLE )
	{
		P3_Simulator::ResetStuckOffsets( player );
		return 0;
	}
	...
	if ( developer.GetBool() )
	{
		...
		engine->Con_NPrintf( isServer, "%s stuck on object %i/%s", 
			...
			hitent.GetEntryIndex(), m_MoveHelper.GetName(hitent) );
	}
#endif
	...
	if ( !player->IsServer() )
	{
		if ( m_MoveHelper.IsWorldEntity( hitent ) )
		{
			int nReps = 0;
			P3_Simulator::ResetStuckOffsets( player );
			do 
			{
				P3_Simulator::GetRandomStuckOffsets( player, offset );
				VectorAdd( base, offset, test );
				
				if ( TestPlayerPosition( test, m_nMovementCollisionGroup, traceresult ) == INVALID_ENTITY_HANDLE )
				{
					P3_Simulator::ResetStuckOffsets( player );
	...
	fTime = engine->Time();
	// Too soon?
	if ( m_flStuckCheckTime[ idx ] >=  fTime - CHECKSTUCK_MINTIME )
	{
		return 1;
	}
	m_flStuckCheckTime[ idx ] = fTime;

	m_MoveHelper.AddToTouched( traceresult, mv->m_vecVelocity );
	P3_Simulator::GetRandomStuckOffsets( player, offset );
	VectorAdd( base, offset, test );
...
// Output : bool
//-----------------------------------------------------------------------------
bool CP3_SimulatorGameMovement::InWater( void )
{
	return ( player->GetWaterLevel() > WL_Feet );
}


void CP3_SimulatorGameMovement::ResetGetPointContentsCache()
{
	for ( int slot = 0; slot < MAX_PC_CACHE_SLOTS; ++slot )
	{
		m_CachedGetPointContents[ slot ] = -9999;
	}
}


int CP3_SimulatorGameMovement::GetPointContentsCached( const Vector &point, int slot )
{
	if ( g_bMovementOptimizations ) 
	{
		Assert( player );
		...
		if ( m_CachedGetPointContents[ slot ] == -9999 || point.DistToSqr( m_CachedGetPointContentsPoint[ slot ] ) > 1 )
		{
			m_CachedGetPointContents[ slot ] = enginetrace->GetPointContents ( point );
			m_CachedGetPointContentsPoint[ slot ] = point;
		}
		
		return m_CachedGetPointContents[ slot ];
	}
	else
	{
...
// Output : bool
//-----------------------------------------------------------------------------
bool CP3_SimulatorGameMovement::CheckWater( void )
{
	Vector	point;
	int		cont;
...
void CP3_SimulatorGameMovement::SetGroundEntity( trace_t *pm )
{
...
//-----------------------------------------------------------------------------
void TraceSimulatorBBoxForGround( const Vector& start, const Vector& end, const Vector& minsSrc,
							  const Vector& maxsSrc, IHandleEntity *player, unsigned int fMask,
							  int collisionGroup, trace_t& pm )
{
	AI_PROFILE_SCOPE( "TracePlayerBBoxForGround" );
...
// Input  : &input - 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::CategorizePosition( void )
{
	return;
...
#define NON_JUMP_VELOCITY 140.0f
	...
	{
		AI_PROFILE_SCOPE_BEGIN( "CP3_SimulatorGameMovement::CategorizePosition" );
		// Try and move down.
		//TracePlayerPrimitive( bumpOrigin, point, MASK_PLAYERSOLID, m_nMovementCollisionGroup, pm );

		//
		Vector shift_up(0,0,10);		
		VectorRotate( shift_up, player->EntityToWorldTransform(), shift_up );

		Vector point1 =bumpOrigin+shift_up;

		Vector end_point( 0, 0, 30 );		
		VectorRotate( end_point, player->EntityToWorldTransform(), end_point );

		UTIL_TraceLine( point1, point1 - end_point, MASK_PLAYERSOLID_BRUSHONLY, player, COLLISION_GROUP_PLAYER_MOVEMENT, &pm );
		//
		...
		//TracePlayerBBoxForGround( bumpOrigin, point, GetPlayerMins(), GetPlayerMaxs(), mv->m_nPlayerHandle.Get(), MASK_PLAYERSOLID, MOVEMENT_COLLISION_GROUP, pm );
		if ( !pm.m_pEnt || pm.plane.normal[2] < 0.7 )
		{
			SetGroundEntity( NULL );
...
void CP3_SimulatorGameMovement::CategorizePosition( trace_t &pm )
{
	Vector point;
...
#define NON_JUMP_VELOCITY 140.0f
	...
	else
	{
		...
		// Test four sub-boxes, to see if any of them would have found shallower slope we could actually stand on
		//TracePlayerBBoxForGround( bumpOrigin, point, GetPlayerMins(), GetPlayerMaxs(), mv->m_nPlayerHandle.Get(), MASK_PLAYERSOLID, MOVEMENT_COLLISION_GROUP, pm );
		if ( !pm.m_pEnt || pm.plane.normal[2] < 0.7f || pm.fraction>0.9f )
		{
			//NDebugOverlay::Box(player->GetAbsOrigin(), Vector(-3,-3,-3), Vector(3,3,3), 255,0,0,255, 3.0f);
			SetGroundEntity( NULL );
			// probably want to add a check for a +z velocity too!
			if ( ( mv->m_vecVelocity.z > 0.0f ) && 
				( player->GetMoveType() != MOVETYPE_NOCLIP ) )
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::CheckFalling( void )
{
	if ( player->GetGroundEntity() != NULL &&
		 !IsDead() &&
		 player->m_Local.m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHOLD )
	{
		...
		else
		{
			...
			if ( player->m_Local.m_flFallVelocity > PLAYER_MAX_SAFE_FALL_SPEED )
			{
				...
				//
				bAlive = m_MoveHelper.PlayerFallingDamage();
				fvol = 1.0;
			}
		...
		if (bAlive)
		{
			m_MoveHelper.PlayerSetAnimation( PLAYER_WALK );
		}
	}
...
void CP3_SimulatorGameMovement::PlayerRoughLandingEffects( float fvol )
{
	if ( fvol > 0.0 )
	{
		//
...
// Output : float
//-----------------------------------------------------------------------------
float CP3_SimulatorGameMovement::SplineFraction( float value, float scale )
{
	float valueSquared;
...
// Input  : direction - 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FixPlayerCrouchStuck( bool upward )
{
	EntityHandle_t hitent;
	int i;
	Vector test;
	...
	hitent = TestPlayerPosition( mv->GetAbsOrigin(), m_nMovementCollisionGroup, dummy );
	if (hitent == INVALID_ENTITY_HANDLE )
		return;
...
bool CP3_SimulatorGameMovement::CanUnduck()
{
	int i;
	trace_t trace;
	Vector newOrigin;
	...
	TracePlayerPrimitive( mv->GetAbsOrigin(), newOrigin, PlayerSolidMask(), m_nMovementCollisionGroup, trace );
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FinishUnDuck( void )
{
	int i;
	trace_t trace;
	Vector newOrigin;
...
// 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::UpdateDuckJumpEyeOffset( void )
{
	if ( player->m_Local.m_flDuckJumpTime != 0.0f )
	{
...
// Purpose:
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FinishUnDuckJump( trace_t &trace )
{
	Vector vecNewOrigin;
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FinishDuck( void )
{
	int i;
...
// Purpose:
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::StartUnDuckJump( void )
{
	player->AddFlag( FL_DUCKING );
	player->m_Local.m_bDucked = true;
	player->m_Local.m_bDucking = false;
...
// Input  : duckFraction - 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::SetDuckedEyeOffset( float duckFraction )
{
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::HandleDuckingSpeedCrop( void )
{
...
//-----------------------------------------------------------------------------
bool CP3_SimulatorGameMovement::CanUnDuckJump( trace_t &trace )
{
	...
	TracePlayerPrimitive( mv->GetAbsOrigin(), vecEnd, PlayerSolidMask(), m_nMovementCollisionGroup, trace );
	if ( trace.fraction < 1.0f )
	{
		...
		player->m_Local.m_bDucked = false;
		TracePlayerPrimitive( vecEnd, vecEnd, PlayerSolidMask(), m_nMovementCollisionGroup, traceUp );
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::Duck( void )
{
...
#ifdef P3_DLL
	bool bDuckJump = ( player->m_Local.m_flJumpTime > 0.0f ) && bInDuck;
#else
...
static ConVarRef sv_optimizedmovement( "sv_optimizedmovement" );

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::PlayerMove( void )
{
	AI_PROFILE_SCOPE( "CP3_SimulatorGameMovement::PlayerMove" );

	CheckParameters();

	/*char tempstr[512];
	Q_snprintf( tempstr,sizeof(tempstr), "sim speed: %f\n", mv->m_flMaxSpeed );			
	Msg( "%s", tempstr );*/	
	...
	m_MoveHelper.ResetTouchList();                    // Assume we don't touch anything
...
#ifndef P3_DLL
	// новая политика партии - отключаем приседания.
	...
	switch (player->GetMoveType())
	{
		...
		case MOVETYPE_STEP:
		case MOVETYPE_WALK:			
			if ( player->GetDriver()!=NULL || !mv->m_vecVelocity.IsZero() )
				FullWalkMove();
			break;
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::PerformFlyCollisionResolution( trace_t &pm, Vector &move )
{
	Vector base;
	float vel;
	float backoff;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::FullTossMove( void )
{
	trace_t pm;
	Vector move;
...
void CP3_SimulatorGameMovement::IsometricMove( void )
{
	int i;
	Vector wishvel;
	float fmove, smove;
...
bool CP3_SimulatorGameMovement::GameHasLadders() const
{
	return true;
}

bool CTraceFilterIgnoreNPC::ShouldHitEntity( IHandleEntity *pHandleEntity, int contentsMask )
{
	if (!m_pMe) 
		return false;
	...
	if (m_pMe == pEntity->GetMoveParent())
		return false;

	CP3_NPC *npc = pEntity->MyP3NPCPointer();
	
	if (npc && npc->GetCollisionGroup() == COLLISION_GROUP_BREAKABLE_BY_VEHICLE )
	{			
		return false;
	}
...
//-----------------------------------------------------------------------------
void CP3_SimulatorGameMovement::StayOnGround( void )
{
	trace_t trace;
	Vector start( mv->GetAbsOrigin() );
	Vector end( mv->GetAbsOrigin() );
	start.z += 2;
	end.z -= 150; //player->GetStepSize();

	// See how far up we can go without getting stuck

	//TracePlayerPrimitive( mv->GetAbsOrigin(), start, PlayerSolidMask(), COLLISION_GROUP_PLAYER_MOVEMENT, trace );
	//start = trace.endpos;
	...
	TracePlayerPrimitive( start, end, PlayerSolidMask(), COLLISION_GROUP_PLAYER_MOVEMENT, trace );
	P3_Simulator::DrawTraceCollision( trace );
...
//void CP3_SimulatorGameMovement::SetHull( const Vector &vecHullMins, const Vector &vecHullMaxs )
//{
//	//m_vecHullMins = vecHullMins;
//	//m_vecHullMaxs =	vecHullMaxs;
//}
