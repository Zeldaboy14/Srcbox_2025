...
// Thanks Adam for the rotational code, and TheMaster974 
// for the fix back in Singleplayer!
//
// $NoKeywords: $
//=============================================================================//
...
#define constraint_ammo false // old constraint ammo from the beta. can't be used with gmod_physgun enabled
#define physgun_usemass false // in beta hl2, the physgun picked up shit with mass. this toggle disables that, akin to gmod
#define gmod_physgun true // freezing and unfreezing objects with the physgun. can't be used with constraint_ammo enabled
#define physgun_audio false // beta audio when the physguns in use. disabled akin to gmod
//#define gmod_physgun_rotatation false // rotational code for the physgun
...
extern void RotateObject(CBasePlayer *pPlayer, float flError);
...
public:
	...
	{
		// add constraint ammo. commenting this out will disable the ammo
		// added a define to this for toggling purposes.
		if (constraint_ammo == true && gmod_physgun == false) {
...
BEGIN_DATADESC(CWeaponGravityGun)
	...
	// adnan
	// bool to say if we're holding something
	DEFINE_FIELD(m_bIsCurrentlyHolding, FIELD_BOOLEAN),
...
void CWeaponGravityGun::EffectUpdate(void)
{
	...
	if (tr.fraction != 1)
	{
		...
		// disabled because gmod doesn't do this
		/*if (distance < 36)
		{
			DetachObject();
			return;
...
void CWeaponGravityGun::DeleteActivePellets()
{
	...
	for (int i = 0; i < m_pelletCount; i++)
	{
		...
		CBeam *pBeam = CBeam::BeamCreate(PHYSGUN_BEAM_SPRITE, 1.5); // latter half is the physgun's beam scale. was set to 1.5 because i thought it was that in gmod
		pBeam->PointEntInit(pPellet->GetAbsOrigin(), pEnt);
		pBeam->SetEndAttachment(1);
		pBeam->SetBrightness(255);
		pBeam->SetColor(255, 0, 0); // todo: solve why this doesn't actually color it. initally 255, 0 , 0.
		pBeam->RelinkBeam();
		pBeam->LiveForTime(1); // initially 0.1
...
void CWeaponGravityGun::AttachObject(CBaseEntity *pObject, const Vector& start, const Vector &end, float distance)
{
	...
	{
		if (gmod_physgun == true && constraint_ammo == false) {
...
{
	//m_flNextPrimaryAttack = gpGlobals->curtime + 0.1; // TODO: is this required anymore????
	if (m_bBlockPrimary) {
		return;
	}
	if (!m_active)
