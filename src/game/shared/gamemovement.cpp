...
// This will set the Half-Life jump height instead of HL2's
#if defined( HL2_DLL ) ||  defined( HL2_CLIENT_DLL )
ConVar srcbox_player_hl1_jumpheight("srcbox_player_hl1_jumpheight", "0", FCVAR_CHEAT, "Enable Half-Life's jump height");
#endif
...
bool CGameMovement::CheckJumpButton( void )
{
	...
	{
	#if defined( HL2_DLL ) || defined( HL2_CLIENT_DLL )
		if (srcbox_player_hl1_jumpheight.GetBool())
		{
			player->SetGravity(1.3333f);
			//Msg("Yo Z! New sv_gravity value: %d\n", sv_gravity.GetInt());
			flMul = 268.3281572999747f;
		}
		else {
			player->SetGravity(0);
