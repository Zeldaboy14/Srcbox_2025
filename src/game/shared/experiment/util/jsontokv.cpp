...
typedef GenericValue< UTF8<> > ValueJSON;

inline void IterObject( const char *pName, ValueJSON::ConstObject obj, KeyValues *kv );

inline void MapNode( ValueJSON::ConstMemberIterator itr, KeyValues *kv )
{
    switch ( itr->value.GetType() )
    {
        ...
        case kArrayType:
        {
            ...
            for ( ValueJSON::ConstValueIterator arrItr = itr->value.Begin(); arrItr != itr->value.End(); ++arrItr )
            {
...
inline void IterObject( const char *pName, ValueJSON::ConstObject obj, KeyValues *kv )
{
    ...
    for ( ValueJSON::ConstMemberIterator itr = obj.MemberBegin(); itr != obj.MemberEnd(); ++itr )
    {
        MapNode( itr, pKv );
    }
}
...
bool CJsonToKeyValues::ConvertJsonToKeyValues( const char *pInput, KeyValues *pOut )
{
    ...
    else
    {
        ...
        for ( ValueJSON::ConstMemberIterator itr = doc.MemberBegin(); itr != doc.MemberEnd(); ++itr )
        {
            MapNode( itr, pOut );
        }
        return true;
