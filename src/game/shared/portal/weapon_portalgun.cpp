...
ConVar	g_debug_portalgun( "g_debug_PortalGun", "0", FCVAR_REPLICATED | FCVAR_CHEAT );
...
ConVar player_throwforce_portalgun("player_throwforce", "1000", FCVAR_REPLICATED | FCVAR_CHEAT);
...
extern ConVar hl2_walkspeed;
	...
	CLIENTEFFECT_MATERIAL( PortalGun_CENTER_GLOW )
	CLIENTEFFECT_MATERIAL( PortalGun_BLAST_SPRITE )
...
void PortalGunBeginUpgrade( CBaseAnimating *pAnim )
{

}

bool PlayerHasMegaPortalGun( void )
{
	return false;
}

bool PortalGunAccountableForObject( CBaseCombatWeapon *pPortalGun, CBaseEntity *pObject )
{
...
BEGIN_SIMPLE_DATADESC( portalgun_shadowcontrol_params_t )
...
float CGrabControllerPortalGun::ComputeError()
{
	...
	if ( pAttached )
	{
		...
		{
			DevMsg( "Object attached to PortalGun has no physics object\n" );
			DetachEntity( false );
...
void CGrabControllerPortalGun::AttachEntity( CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaPortalGun, const Vector &vGrabPosition, bool bUseGrabPosition )
{
...
{
	portalgun_shadowcontrol_params_t shadowParams = m_shadow;
...
public:
	...
	bool PortalGunIsHoldingEntity( CBaseEntity *pEnt );
...
void CPlayerPickupControllerPortalGun::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( ToBasePlayer(pActivator) == m_pPlayer )
	{
		...
		if ( m_pPlayer->m_nButtons & IN_ATTACK )
		{
			...
			vecLaunch *= player_throwforce_portalgun.GetFloat() * massFactor;
...
//-----------------------------------------------------------------------------
bool CPlayerPickupControllerPortalGun::PortalGunIsHoldingEntity( CBaseEntity *pEnt )
{
	return ( m_grabController.GetAttached() == pEnt );
}

void PortalGunPlayerPickupObject( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
...
void CWeaponPortalGun::Precache( void )
{
	...
	PrecacheScriptSound("Weapon_Portalgun.fire_blue_npc");
	PrecacheScriptSound("Weapon_Portalgun.fire_red_npc");
...
void CWeaponPortalGun::OnRestore()
{
	...
	// Tracker 8106:  PortalGun effects disappear through level transition, so
...
#ifdef CLIENT_DLL
void CWeaponPortalGun::OnDataChanged( DataUpdateType_t type )
{
...
void CWeaponPortalGun::PrimaryFireEffect( void )
{
	...
	pOwner->ViewPunch( QAngle(-6, SharedRandomInt( "PortalGunfire", -2,2) ,0) );
...
// Input  : - 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::ApplyVelocityBasedForce( CBaseEntity *pEntity, const Vector &forward )
{
#ifndef CLIENT_DLL
	...
	float flForceMax = portalgun_maxforce.GetFloat();
	...
	{
		mass = MIN(mass, 1000);
		float flForceMin = portalgun_minforce.GetFloat();
...
CWeaponPortalGun::FindObjectResult_t CWeaponPortalGun::FindObject( void )
{
	...
	pullDir *= portalgun_pullforce.GetFloat();
...
#ifdef CLIENT_DLL
void CWeaponPortalGun::ManagePredictedObject( void )
{
...
//-----------------------------------------------------------------------------
void CWeaponPortalGun::UpdateElementPosition( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
void CWeaponPortalGun::ClientThink( void )
{
...
void CWeaponPortalGun::DoEffectIdle( void )
{
	...
	//if ( ShouldDrawUsingViewModel() )
	{
		...
		for ( int i = PortalGun_GLOW1; i < (PortalGun_GLOW1+NUM_GLOW_SPRITES); i++ )
		{
		...
		// Turn on the glow sprites
		for ( int i = PortalGun_ENDCAP1; i < (PortalGun_ENDCAP1+NUM_ENDCAP_SPRITES); i++ )
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
#define PortalGun_DANGER_SOUND_RADIUS 128
...
void CWeaponPortalGun::StartEffects( void )
{
	...
	if ( m_Parameters[PortalGun_CORE].GetMaterial() == NULL )
	{
		m_Parameters[PortalGun_CORE].GetScale().Init( 0.0f, 1.0f, 0.1f );
		m_Parameters[PortalGun_CORE].GetAlpha().Init( 255.0f, 255.0f, 0.1f );
		m_Parameters[PortalGun_CORE].SetAttachment( 1 );
		
		if ( m_Parameters[PortalGun_CORE].SetMaterial( PortalGun_CENTER_GLOW ) == false )
		{
	...
	if ( m_Parameters[PortalGun_BLAST].GetMaterial() == NULL )
	{
		m_Parameters[PortalGun_BLAST].GetScale().Init( 0.0f, 1.0f, 0.1f );
		m_Parameters[PortalGun_BLAST].GetAlpha().Init( 255.0f, 255.0f, 0.1f );
		m_Parameters[PortalGun_BLAST].SetAttachment( 1 );
		m_Parameters[PortalGun_BLAST].SetVisible( false );
		
		if ( m_Parameters[PortalGun_BLAST].SetMaterial( PortalGun_BLAST_SPRITE ) == false )
		{
			// This means the texture was not found
			Assert( 0 );
		}
	...
	for ( int i = PortalGun_GLOW1; i < (PortalGun_GLOW1+NUM_GLOW_SPRITES); i++ )
	{
		...
		if ( ShouldDrawUsingViewModel() )
		{
			m_Parameters[i].SetAttachment( LookupAttachment( attachNamesGlow[i-PortalGun_GLOW1] ) );
		}
		else
		{
			m_Parameters[i].SetAttachment( LookupAttachment( attachNamesGlowThirdPerson[i-PortalGun_GLOW1] ) );
		}
	...
	for ( int i = PortalGun_ENDCAP1; i < (PortalGun_ENDCAP1+NUM_ENDCAP_SPRITES); i++ )
	{
		...
		m_Parameters[i].SetAttachment( LookupAttachment( attachNamesEndCap[i-PortalGun_ENDCAP1] ) );
...
void CWeaponPortalGun::DoEffectReady( void )
{
	...
	if ( ShouldDrawUsingViewModel() )
	{
		...
		m_Parameters[PortalGun_CORE].GetScale().InitFromCurrent( 14.0f, 0.2f );
		m_Parameters[PortalGun_CORE].GetAlpha().InitFromCurrent( 128.0f, 0.2f );
		m_Parameters[PortalGun_CORE].SetVisible();
	}
	else
	{
		//Turn off the center sprite
		m_Parameters[PortalGun_CORE].GetScale().InitFromCurrent( 8.0f, 0.2f );
		m_Parameters[PortalGun_CORE].GetAlpha().InitFromCurrent( 0.0f, 0.2f );
		m_Parameters[PortalGun_CORE].SetVisible();
	}
...
void CWeaponPortalGun::DoEffectHolding( void )
{
	...
	if ( ShouldDrawUsingViewModel() )
	{
		...
		m_Parameters[PortalGun_CORE].GetScale().InitFromCurrent( 16.0f, 0.2f );
		m_Parameters[PortalGun_CORE].GetAlpha().InitFromCurrent( 255.0f, 0.1f );
		m_Parameters[PortalGun_CORE].SetVisible();
		...
		for ( int i = PortalGun_ENDCAP1; i < (PortalGun_ENDCAP1+NUM_ENDCAP_SPRITES-1); i++ )
		{
			m_Parameters[i].SetVisible();
		}
		...
		// Setup the beams
		m_Beams[0].Init( LookupAttachment( "ValveBiped.Arm1_C" ), 1, pBeamEnt, true );
		m_Beams[1].Init( LookupAttachment( "ValveBiped.Arm2_C" ), 1, pBeamEnt, true );
...
void CWeaponPortalGun::DoEffectLaunch( Vector *pos )
{
	...
	te->DispatchEffect( filter, 0.0, data.m_vOrigin, "PortalGunImpact", data );

#ifdef CLIENT_DLL

	//Turn on the blast sprite and scale
	m_Parameters[PortalGun_BLAST].GetScale().Init( 8.0f, 64.0f, 0.1f );
	m_Parameters[PortalGun_BLAST].GetAlpha().Init( 255.0f, 0.0f, 0.2f );
	m_Parameters[PortalGun_BLAST].SetVisible();
...
void CWeaponPortalGun::DoEffectNone( void )
{
	...
	//Turn off main glows
	m_Parameters[PortalGun_CORE].SetVisible( false );
	m_Parameters[PortalGun_BLAST].SetVisible( false );
...
//			effect parameter
//-----------------------------------------------------------------------------
void CWeaponPortalGun::GetEffectParameters( EffectType_t effectID, color32 &color, float &scale, IMaterial **pMaterial, Vector &vecAttachment )
{
...
//-----------------------------------------------------------------------------
bool CWeaponPortalGun::IsEffectVisible( EffectType_t effectID )
{
...
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DrawEffectSprite( EffectType_t effectID )
{
	color32 color;
	float scale;
	IMaterial *pMaterial;
...
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DrawEffects( void )
{
	// Draw the core effects
	//DrawEffectSprite( PortalGun_CORE );
	//DrawEffectSprite( PortalGun_BLAST );
...
//-----------------------------------------------------------------------------
int CWeaponPortalGun::DrawModel( int flags )
{
...
//-----------------------------------------------------------------------------
void CWeaponPortalGun::ViewModelDrawn( C_BaseViewModel *pBaseViewModel )
{
	// Render our effects
	DrawEffects();
...
//-----------------------------------------------------------------------------
bool CWeaponPortalGun::IsTransparent( void )
{
	return true;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::NotifyShouldTransmit( ShouldTransmitState_t state )
{
...
bool PlayerPickupControllerIsHoldingEntityPortalGun( CBaseEntity *pPickupControllerEntity, CBaseEntity *pHeldEntity )
{
	CPlayerPickupControllerPortalGun *pController = dynamic_cast<CPlayerPickupControllerPortalGun *>(pPickupControllerEntity);

	return pController ? pController->PortalGunIsHoldingEntity( pHeldEntity ) : false;
}
...
float PortalGunPlayerPickupGetHeldObjectMass( CBaseEntity *pPickupControllerEntity, IPhysicsObject *pHeldObject )
{
	float mass = 0.0f;
	CPlayerPickupControllerPortalGun *pController = dynamic_cast<CPlayerPickupControllerPortalGun *>(pPickupControllerEntity);
	if ( pController )
...
void CallbackPortalGunImpact( const CEffectData &data )
{
	...
	if ( pWeapon->GetOwner() == CBasePlayer::GetLocalPlayer() )
	{
		...
		beamInfo.m_pszModelName = PortalGun_BEAM_SPRITE;
...
DECLARE_CLIENT_EFFECT( "PortalGunImpact", CallbackPortalGunImpact );
