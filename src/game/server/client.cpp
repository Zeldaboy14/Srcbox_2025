...
// Console Command to print the VMT. Useful atm...

// Custom function to trim leading and trailing whitespace
...
// Function to ensure no double ".vmt" extension
void FixMaterialPath(char* materialPath, size_t size) {
	const char* extension = Q_stristr(materialPath, ".vmt");
	if (!extension) {
		Q_strncat(materialPath, ".vmt", size - strlen(materialPath) - 1);
	}
}

// Function to print the shader and texture information from a material
void PrintMaterialShaderInfo(const char* materialName) {
	// Get the full path to the material file (.vmt)
	char materialPath[MAX_PATH];
	Q_snprintf(materialPath, sizeof(materialPath), "materials/%s", materialName);

	// Ensure no double ".vmt"
	FixMaterialPath(materialPath, sizeof(materialPath));

	// Check if the material file exists
	if (!filesystem->FileExists(materialPath)) {
		Msg("Error: Material file '%s' not found.\n", materialPath);
		return;
	}

	// Open the material file for reading
	FileHandle_t fileHandle = filesystem->Open(materialPath, "r");
	if (!fileHandle) {
		Msg("Error: Failed to open material file '%s'.\n", materialPath);
		return;
	}

	// Read the shader name (the first line before the '{')
	char line[1024];
	bool foundShaderName = false;
	const char* shaderName = "Unknown Shader";

	while (filesystem->ReadLine(line, sizeof(line), fileHandle)) {
		...
		// The first non-empty line should be the shader name
		if (!foundShaderName) {
			shaderName = line;
			foundShaderName = true;
			Msg("Material '%s' Shader: %s\n", materialName, shaderName);
		}

		// If we encounter '{', start reading the parameters
		if (line[0] == '{') {
			break;
		}
	}

	// Process parameters inside the '{' block
	while (filesystem->ReadLine(line, sizeof(line), fileHandle)) {
		// Trim leading and trailing spaces
		TrimWhitespace(line);
		...
		// If we encounter '}', stop reading parameters
		if (line[0] == '}') {
			break;
		}

		KeyValues* pKeyValues = new KeyValues("Root");

		// Process lines that start with '$' (parameter lines)
		if (pKeyValues->LoadFromFile(filesystem, materialPath)) {
			// The shader name is the first key before the '{' bracket, so get the first key
			KeyValues* pFirstKey = pKeyValues->GetFirstSubKey();

			if (pFirstKey) {
				// The first key should be the shader class, not a parameter like $basetexture.

				// Now, let's check for other parameters like $basetexture and $bumpmap, inside the curly braces
				const char* baseTexture = pKeyValues->GetString("$basetexture", "No texture found");
				Msg("Material '%s' $basetexture: %s\n", materialName, baseTexture);

				const char* bumpMap = pKeyValues->GetString("$bumpmap", "No bumpmap found");
				Msg("Material '%s' $bumpmap: %s\n", materialName, bumpMap);

				const char* envMap = pKeyValues->GetString("$envmap", "No environment map found");
				Msg("Material '%s' $envmap: %s\n", materialName, envMap);

				// Print any other parameters if needed
				const char* phong = pKeyValues->GetString("$phong", "No phong parameter found");
				Msg("Material '%s' $phong: %s\n", materialName, phong);

				const char* detail = pKeyValues->GetString("$detail", "No detail texture found");
				Msg("Material '%s' $detail: %s\n", materialName, detail);

				break;

				// Add other parameters you need here
			}
			else {
				Msg("Material '%s' does not have any keys.\n", materialName);
			}
		}
	}
...
void PrintModelMaterialShaderInfo(const char* modelName) {
	// Get the model index using the model name
	int modelIndex = modelinfo->GetModelIndex(modelName);

	if (modelIndex == -1) {
		Msg("Error: Model '%s' not found.\n", modelName);
		return;
	}

	// Get the model pointer using the model index
	...
	// Get the studio model associated with this model
	const studiohdr_t* pStudioHdr = modelinfo->GetStudiomodel(pModel);

	if (!pStudioHdr) {
		Msg("Error: Studio model for '%s' not found.\n", modelName);
		return;
	}

	// Access the texture information from the studio model
	mstudiotexture_t* pTextures = (mstudiotexture_t*)((byte*)pStudioHdr + pStudioHdr->textureindex);

	// Loop through all textures/materials used by the model
	for (int i = 0; i < pStudioHdr->numtextures; i++) {
		const char* relativePath = pTextures[i].pszName();

		if (relativePath) {
			// Construct the full material path based on the embedded relative path
			char fullMaterialPath[MAX_PATH];


			Q_snprintf(fullMaterialPath, sizeof(fullMaterialPath), "materials/%s.vmt", "mstudiotexture_t");

			// Log the resolved material path
			if (filesystem->FileExists(fullMaterialPath)) {
				Msg("Resolved Material Path: %s\n", fullMaterialPath);
			}
			else {
				Msg("Material file missing: %s\n", fullMaterialPath);
			}

			// Open the material file to extract shader and texture information
			FileHandle_t fileHandle = filesystem->Open(fullMaterialPath, "r");
			if (!fileHandle) {
				//Msg("Error: Failed to open material file '%s'.\n", fullMaterialPath);
				Msg("Error: Failed to open material file '%s'.\n", fullMaterialPath);
				continue;
			...
			// Read shader name and parameters
			char line[1024];
			bool foundShaderName = false;
			const char* shaderName = "Unknown Shader";
			...
			while (filesystem->ReadLine(line, sizeof(line), fileHandle)) {
				...
				// The first non-empty line is typically the shader name
				if (!foundShaderName) {
					shaderName = line;
					foundShaderName = true;
					Msg("Material '%s' Shader: %s\n", relativePath, shaderName);
				}

				// Start processing parameters when '{' is encountered
				if (line[0] == '{') {
					break;
				}
			}
			...
			while (filesystem->ReadLine(line, sizeof(line), fileHandle)) {
				...
				// Stop processing on encountering '}'
				if (line[0] == '}') {
					break;
				}

				// Extract specific material parameters
				if (Q_strnicmp(line, "$basetexture", 12) == 0) {
					Msg("Material '%s' $basetexture: %s\n", relativePath, line + 13);
				}
				else if (Q_strnicmp(line, "$bumpmap", 8) == 0) {
					Msg("Material '%s' $bumpmap: %s\n", relativePath, line + 9);
				}
				else if (Q_strnicmp(line, "$envmap", 8) == 0) {
					Msg("Material '%s' $envmap: %s\n", relativePath, line + 9);
				}
				else if (Q_strnicmp(line, "$phong", 6) == 0) {
					Msg("Material '%s' $phong: %s\n", relativePath, line + 7);
				}
				else if (Q_strnicmp(line, "$detail", 7) == 0) {
					Msg("Material '%s' $detail: %s\n", relativePath, line + 8);
				}
			}
			...
			Msg("Error: Failed to retrieve material path for model '%s'.\n", modelName);
		}
	}
}


// Console command to print either model or material shader info
CON_COMMAND(srcbox_material_print, "Prints shader and texture information for a model or material.") {
	if (args.ArgC() < 2) {
		Msg("Usage:srcbox_vmt_print <model_name.mdl|material_name.vmt>\n");
		return;
	}

	const char* input = args.Arg(1);

	// Check if the input ends with ".vmt"
	if (Q_stristr(input, ".vmt") != nullptr) {
		PrintMaterialShaderInfo(input); // Print material info
	}
	else {
		PrintModelMaterialShaderInfo(input); // Print model info
	}
}

// End VMT Reader

// mounting games start
