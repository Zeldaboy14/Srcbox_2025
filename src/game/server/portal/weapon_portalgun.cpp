...
ConVar portalgun_minforce( "portalgun_minforce", "700" );
ConVar portalgun_maxforce( "portalgun_maxforce", "1500" );
ConVar portalgun_maxmass( "portalgun_maxmass", "85" );
ConVar portalgun_tracelength( "portalgun_tracelength", "250" );
ConVar portalgun_mega_tracelength( "portalgun_mega_tracelength", "850" );
ConVar portalgun_chargetime("portalgun_chargetime", "2" );
ConVar portalgun_pullforce( "portalgun_pullforce", "4000" );
ConVar portalgun_mega_pullforce( "portalgun_mega_pullforce", "8000" );
ConVar portalgun_cone( "portalgun_cone", "0.97" );
ConVar portalgun_ball_cone( "portalgun_ball_cone", "0.997" );
ConVar portalgun_punt_cone( "portalgun_punt_cone", "0.997" );
ConVar portalgun_player_throwforce( "portalgun_player_throwforce", "1000" );
ConVar portalgun_dmg_glass( "portalgun_dmg_glass", "15" );
ConVar portalgun_right_turrets( "portalgun_right_turrets", "0" );

extern ConVar hl2_normspeed;
extern ConVar hl2_walkspeed;

#define PHYSCANNON_BEAM_SPRITE "sprites/orangelight1_noz.vmt"
...
class CTraceFilterPortalGun : public CTraceFilterSimple
{
public:
	DECLARE_CLASS( CTraceFilterPortalGun, CTraceFilterSimple );

	CTraceFilterPortalGun( const IHandleEntity *passentity, int collisionGroup )
...
//-----------------------------------------------------------------------------
void UTIL_PortalGunTraceLine( const Vector &vecAbsStart, const Vector &vecAbsEnd, CBaseEntity *pTraceOwner, trace_t *pTrace )
{
	...
	CTraceFilterPortalGun filter( pTraceOwner, COLLISION_GROUP_NONE );
	UTIL_TraceLine( vecAbsStart, vecAbsEnd, (MASK_SHOT|CONTENTS_GRATE), &filter, pTrace );
...
//-----------------------------------------------------------------------------
void UTIL_PortalGunTraceHull( const Vector &vecAbsStart, const Vector &vecAbsEnd, const Vector &vecAbsMins, const Vector &vecAbsMaxs, CBaseEntity *pTraceOwner, trace_t *pTrace )
{
	// Default to HL2 vanilla
	if ( hl2_episodic.GetBool() == false )
	{
...
//-----------------------------------------------------------------------------
IPhysicsObject *PortalGetRagdollChildAtPosition( CBaseEntity *pTarget, const Vector &position )
{
	// Check for a ragdoll
...
struct game_shadowcontrol_params_tpg : public hlshadowcontrol_params_t
{
	DECLARE_SIMPLE_DATADESC();
};

BEGIN_SIMPLE_DATADESC( game_shadowcontrol_params_tpg )
...
//-----------------------------------------------------------------------------
class CGrabControllerPortalGun : public IMotionEvent
{
	DECLARE_SIMPLE_DATADESC();

public:

	CGrabControllerPortalGun( void );
	~CGrabControllerPortalGun( void );
...
private:
	...
	game_shadowcontrol_params_tpg	m_shadow;
	...
	friend class CWeaponPortalGun;
};

BEGIN_SIMPLE_DATADESC( CGrabControllerPortalGun )
...
CGrabControllerPortalGun::CGrabControllerPortalGun( void )
{
...
void CGrabControllerPortalGun::OnRestore()
{
	if ( m_controller )
	{
...
void CGrabControllerPortalGun::SetTargetPosition( const Vector &target, const QAngle &targetOrientation )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CGrabControllerPortalGun::ComputeError()
{
	if ( m_errorTime <= 0 )
		return 0;
...
void CGrabControllerPortalGun::ComputeMaxSpeed( CBaseEntity *pEntity, IPhysicsObject *pPhysics )
{
	m_shadow.maxSpeed = 1000;
	m_shadow.maxAngular = DEFAULT_MAX_ANGULAR;
	...
	float flMaxMass = portalgun_maxmass.GetFloat();
	if ( flMass <= flMaxMass )
		return;
...
QAngle CGrabControllerPortalGun::TransformAnglesToPlayerSpace( const QAngle &anglesIn, CBasePlayer *pPlayer )
{
...
QAngle CGrabControllerPortalGun::TransformAnglesFromPlayerSpace( const QAngle &anglesIn, CBasePlayer *pPlayer )
{
	if ( m_bIgnoreRelativePitch )
	{
		matrix3x4_t test;
...
void CGrabControllerPortalGun::AttachEntity( CBasePlayer *pPlayer, CBaseEntity *pEntity, IPhysicsObject *pPhys, bool bIsMegaPhysCannon, const Vector &vGrabPosition, bool bUseGrabPosition )
{
...
//	ComputeMaxSpeed( pEntity, pPhys );
	...
	if ( bUseGrabPosition )
	{
		IPhysicsObject *pChild = PortalGetRagdollChildAtPosition( pEntity, vGrabPosition );
...
void CGrabControllerPortalGun::DetachEntity( bool bClearVelocity )
{
...
IMotionEvent::simresult_e CGrabControllerPortalGun::Simulate( IPhysicsMotionController *pController, IPhysicsObject *pObject, float deltaTime, Vector &linear, AngularImpulse &angular )
{
	game_shadowcontrol_params_tpg shadowParams = m_shadow;
...
float CGrabControllerPortalGun::GetSavedMass( IPhysicsObject *pObject )
{
...
//-----------------------------------------------------------------------------
bool CGrabControllerPortalGun::IsObjectAllowedOverhead( CBaseEntity *pEntity )
{
...
//-----------------------------------------------------------------------------
class CPlayerPickupControllerPortalGun : public CBaseEntity
{
	DECLARE_DATADESC();
	DECLARE_CLASS( CPlayerPickupControllerPortalGun, CBaseEntity );
public:
	...
	CGrabControllerPortalGun &GetGrabController() { return m_grabController; }

private:
	CGrabControllerPortalGun		m_grabController;
	CBasePlayer			*m_pPlayer;
};

LINK_ENTITY_TO_CLASS( player_pickup, CPlayerPickupControllerPortalGun );

//---------------------------------------------------------
// Save/Restore
//---------------------------------------------------------
BEGIN_DATADESC( CPlayerPickupControllerPortalGun )
...
//			*pObject - 
//-----------------------------------------------------------------------------
void CPlayerPickupControllerPortalGun::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
...
// Input  : bool - 
//-----------------------------------------------------------------------------
void CPlayerPickupControllerPortalGun::Shutdown( bool bThrown )
{
...
void CPlayerPickupControllerPortalGun::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	if ( ToBasePlayer(pActivator) == m_pPlayer )
	{
		...
		if ( m_pPlayer->m_nButtons & IN_ATTACK )
		{
			...
			vecLaunch *= portalgun_player_throwforce.GetFloat() * massFactor;
...
//-----------------------------------------------------------------------------
bool CPlayerPickupControllerPortalGun::IsHoldingEntity( CBaseEntity *pEnt )
{
	return ( m_grabController.GetAttached() == pEnt );
}

void PortalPlayerPickupObject( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
	//Don't pick up if we don't have a phys object.
	if ( pObject->VPhysicsGetObject() == NULL )
		 return;

	CPlayerPickupControllerPortalGun *pController = (CPlayerPickupControllerPortalGun *)CBaseEntity::Create( "player_pickup", pObject->GetAbsOrigin(), vec3_angle, pPlayer );
...
BEGIN_SIMPLE_DATADESC( thrown_objects_tpg )
...
protected:
	...
	CHandle<CSprite>	m_hBodyGlowSprites[NUM_SPRITES];
	...
	CUtlVector< thrown_objects_tpg >	m_ThrownEntities;
...
bool CWeaponPortalGun::m_sbStaticPoseParamsLoaded = false;
int CWeaponPortalGun::m_poseActive = 0;
...
// Sprite scale factor 
//-----------------------------------------------------------------------------
inline float CWeaponPortalGun::SpriteScaleFactor() 
{
...
//-----------------------------------------------------------------------------
void CWeaponPortalGun::ForceDrop( void )
{
	CloseElements();
	DetachObject();
	StopEffects();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPortalGun::DropIfEntityHeld( CBaseEntity *pTarget )
{
	if ( pTarget == NULL )
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::PrimaryFireEffect( void )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponPortalGun::PuntNonVPhysics( CBaseEntity *pEntity, const Vector &forward, trace_t &tr )
{
	float flDamage = 1.0f;
	if ( FClassnameIs( pEntity, "func_breakable" ) )
	{
		...
		{
			flDamage = portalgun_dmg_glass.GetFloat();
		}
	}
...
// What happens when the physgun picks up something 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::Physgun_OnPhysGunPickup( CBaseEntity *pEntity, CBasePlayer *pOwner, PhysGunPickup_t reason )
{
	// If the target is debris, convert it to non-debris
...
// Punt vphysics
//-----------------------------------------------------------------------------
void CWeaponPortalGun::PuntVPhysics( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	CTakeDamageInfo	info;
...
// Input  : - 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::ApplyVelocityBasedForce( CBaseEntity *pEntity, const Vector &forward, const Vector &vecHitPos, PhysGunForce_t reason )
{
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponPortalGun::PuntRagdoll( CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr )
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
// Trace length
//-----------------------------------------------------------------------------
float CWeaponPortalGun::TraceLength()
{
	if ( !IsMegaPhysCannon() )
	{
		return portalgun_tracelength.GetFloat();
	}
	
	return portalgun_mega_tracelength.GetFloat();
}
...
//-----------------------------------------------------------------------------
bool CWeaponPortalGun::EntityAllowsPunts( CBaseEntity *pEntity )
{
...
void CWeaponPortalGun::PrimaryAttack( void )
{
	...
	if( m_bActive )
	{
		...
		if ( pHeld != NULL )
		{
			...
			if ( heldDist > portalgun_tracelength.GetFloat() )
			{
		...
		LaunchObject( forward, portalgun_maxforce.GetFloat() );
	...
	float flPuntDistance = portalgun_tracelength.GetFloat();
	VectorMA( start, flPuntDistance, forward, end );

	trace_t tr;
	UTIL_PortalGunTraceHull( start, end, -Vector(8,8,8), Vector(8,8,8), pOwner, &tr );
	bool bValid = true;
	...
	if ( !bValid )
	{
		UTIL_PortalGunTraceLine( start, end, pOwner, &tr );
		if ( tr.fraction == 1 || !tr.m_pEnt || tr.m_pEnt->IsEFlagSet( EFL_NO_PHYSCANNON_INTERACTION ) )
		{
			if( hl2_episodic.GetBool() )
			{
				// Try to find something in a very small cone. 
				CBaseEntity *pObject = FindObjectInCone( start, forward, portalgun_punt_cone.GetFloat() );

				if( pObject )
				{
					// Trace to the object.
					UTIL_PortalGunTraceLine( start, pObject->WorldSpaceCenter(), pOwner, &tr );
...
{
	// Portal doesn't do this. Replace!
	...
	if ( ( m_bActive ) && ( pOwner->m_afButtonPressed & IN_USE ) )
	{
...
// Input  : *pObject - 
//-----------------------------------------------------------------------------
bool CWeaponPortalGun::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
	if ( m_bActive )
		return false;
...
#if defined(HL2_DLL)
	if( portalgun_right_turrets.GetBool() && pObject->ClassMatches("npc_turret_floor") )
	{
...
void CWeaponPortalGun::FindObjectTrace( CBasePlayer *pPlayer, trace_t *pTraceResult )
{
	Vector forward;
	...
	UTIL_PortalGunTraceLine( start, end, pPlayer, pTraceResult );

	// Try again with a hull trace
	if ( !pTraceResult->DidHitNonWorldEntity() )
	{
		UTIL_PortalGunTraceHull( start, end, -Vector(4,4,4), Vector(4,4,4), pPlayer, pTraceResult );
	}
}


CWeaponPortalGun::FindObjectResult_t CWeaponPortalGun::FindObject( void )
{
	...
	if ( !IsMegaPhysCannon() )
	{
		if (!bAttach && !bPull)
		{
			pConeEntity = FindObjectInCone( start, forward, portalgun_cone.GetFloat() );
		}
	}
	else
	{
		pConeEntity = MegaPhysCannonFindObjectInCone( start, forward, 
			portalgun_cone.GetFloat(), portalgun_ball_cone.GetFloat(), bAttach || bPull );
	}
	...
	pullDir *= IsMegaPhysCannon() ? portalgun_mega_pullforce.GetFloat() : portalgun_pullforce.GetFloat();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponPortalGun::MegaPhysCannonFindObjectInCone( const Vector &vecOrigin, 
   const Vector &vecDir, float flCone, float flCombineBallCone, bool bOnlyCombineBalls )
{
	...
	for( int i = 0 ; i < count ; i++ )
	{
		...
		trace_t tr;
		UTIL_PortalGunTraceLine( vecOrigin, list[ i ]->WorldSpaceCenter(), GetOwner(), &tr );
		if( tr.m_pEnt == list[ i ] )
		{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponPortalGun::FindObjectInCone( const Vector &vecOrigin, const Vector &vecDir, float flCone )
{
	// Find the nearest physics-based item in a cone in front of me.
	CBaseEntity *list[256];
	float flNearestDist = portalgun_tracelength.GetFloat() + 1.0; //Use regular distance.
	Vector mins = vecOrigin - Vector( flNearestDist, flNearestDist, flNearestDist );
	Vector maxs = vecOrigin + Vector( flNearestDist, flNearestDist, flNearestDist );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CGrabControllerPortalGun::UpdateObject( CBasePlayer *pPlayer, float flError )
{
 	CBaseEntity *pEntity = GetAttached();
...
void CWeaponPortalGun::UpdateObject( void )
{
	CBasePlayer *pPlayer = ToBasePlayer( GetOwner() );
	Assert( pPlayer );
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DetachObject( bool playSound, bool wasLaunched )
{
	if ( m_bActive == false )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::CheckForTarget( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoEffectIdle( void )
{
	if ( IsEffectActive( EF_NODRAW ) )
	{
		StopEffects();
...
void CWeaponPortalGun::LaunchObject( const Vector &vecDir, float flForce )
{
	// FIRE!!!
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPortalGun::CanPickupObject( CBaseEntity *pTarget )
{
	if ( pTarget == NULL )
		return false;
	...
	if ( !IsMegaPhysCannon() )
	{
		if ( pTarget->VPhysicsIsFlesh( ) )
			return false;
		return CBasePlayer::CanPickupObject( pTarget, portalgun_maxmass.GetFloat(), 0 );
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::OpenElements( void )
{
	if ( m_bOpen )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::CloseElements( void )
{
...
// Output : float
//-----------------------------------------------------------------------------
float CWeaponPortalGun::GetLoadPercentage( void )
{
	float loadWeight = m_grabController.GetLoadWeight();
	loadWeight /= portalgun_maxmass.GetFloat();	
...
// Output : CSoundPatch
//-----------------------------------------------------------------------------
CSoundPatch *CWeaponPortalGun::GetMotorSound( void )
{
	if ( m_sndMotor == NULL )
	{
...
void CWeaponPortalGun::StartEffects( void )
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		m_hBeams[i] = CBeam::BeamCreate( PHYSCANNON_BEAM_SPRITE, 0.0f );
		...
		m_hBeams[i]->SetBrightness( 32 );
		m_hBeams[i]->SetWidth( 0 );
		m_hBeams[i]->SetEndWidth( random->RandomFloat( 2, 1 ) );
	}
	...
	for ( i = 0; i < NUM_SPRITES; i++ )
	{
		...
		m_hGlowSprites[i] = CSprite::SpriteCreate( PHYSCANNON_GLOW_SPRITE, GetAbsOrigin(), false );

		m_hGlowSprites[i]->SetAsTemporary();

		m_hGlowSprites[i]->SetAttachment( pOwner->GetViewModel(), LookupAttachment(attachNames[i] ) );

		m_hGlowSprites[i]->SetTransparency(kRenderTransAdd, 255, 128, 0, 128, kRenderFxNoDissipation);

		m_hGlowSprites[i]->SetBrightness( 255, 0.4f );
		m_hGlowSprites[i]->SetScale( 0.25f, 0.05f );
	}

	//Create the glow sprites (for portal gun (Body Light)
	for (i = 0; i < 1; i++)
	{
		if (m_hBodyGlowSprites[i])
			continue;

		const char* bodyattachNames[] =
		{
			"Body_light",
		};

		m_hBodyGlowSprites[i] = CSprite::SpriteCreate(MEGACANNON_GLOW_SPRITE, GetAbsOrigin(), false);

		m_hBodyGlowSprites[i]->SetAsTemporary();

		m_hBodyGlowSprites[i]->SetAttachment(pOwner->GetViewModel(), LookupAttachment(bodyattachNames[i]));

		m_hBodyGlowSprites[i]->SetTransparency(kRenderTransAdd, 255, 128, 0, 128, kRenderFxNoDissipation);

		m_hBodyGlowSprites[i]->SetBrightness(255, 0.8f);
		m_hBodyGlowSprites[i]->SetScale(0.05f, 0.025f);
	}
...
// Closing effects
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoEffectClosed( void )
{
	float flScaleFactor = SpriteScaleFactor();
	...
	for (int i = 0; i < 1; i++)
	{
		if (m_hBodyGlowSprites[i] != NULL)
		{
			m_hBodyGlowSprites[i]->TurnOn();
			m_hBodyGlowSprites[i]->SetBrightness(16.0f, 0.2f);
			m_hBodyGlowSprites[i]->SetScale(0.3f * flScaleFactor, 0.2f);
		}
	}
...
// Closing effects
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoMegaEffectClosed( void )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Ready effects
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoEffectReady( )
{
	float flScaleFactor = SpriteScaleFactor();
	...
	for ( int i = 0; i < NUM_SPRITES; i++ )
	{
		if ( m_hGlowSprites[i] != NULL )
		{
			...
			m_hGlowSprites[i]->SetScale( 0.1f * flScaleFactor, 0.2f );
		}
	}
...
// Holding effects
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoEffectHolding( )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Launch effects
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoEffectLaunch( Vector *pos )
{
	Assert( pos );
	if ( pos == NULL )
		return;
...
// Input  : *pos - 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoMegaEffectLaunch( Vector *pos )
{
	Assert( pos );
	if ( pos == NULL )
		return;
	...
	CBeam* pBeam = CBeam::BeamCreate(PHYSCANNON_BEAM_SPRITE, 0.8);
	...
	{
		pBeam = CBeam::BeamCreate( PHYSCANNON_BEAM_SPRITE, 0.8 );
...
// Holding effects
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoMegaEffectHolding( void )
{
	float flScaleFactor = SpriteScaleFactor();
...
// Ready effects
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoMegaEffectReady( void )
{
	float flScaleFactor = SpriteScaleFactor();
...
//			*pos - 
//-----------------------------------------------------------------------------
void CWeaponPortalGun::DoMegaEffect( int effectType, Vector *pos )
{
	switch( effectType )
	{
	case EFFECT_CLOSED:
...
// Output : const char
//-----------------------------------------------------------------------------
const char *CWeaponPortalGun::GetShootSound( int iIndex ) const
{
	...
	case SPECIAL2:
		return "Portal.fizzle_invalid_surface";
		break;
...
//-----------------------------------------------------------------------------
void CWeaponPortalGun::RecordThrownObject( CBaseEntity *pObject )
{
	thrown_objects_tpg thrown;
...
void CWeaponPortalGun::PurgeThrownObjects()
{
	bool bListChanged;
...
bool CWeaponPortalGun::IsAccountableForObject( CBaseEntity *pObject )
{
...
// EXTERNAL API
//-----------------------------------------------------------------------------
void PortalGunForceDrop( CBaseCombatWeapon *pActiveWeapon, CBaseEntity *pOnlyIfHoldingThis )
{
	CWeaponPortalGun *pCannon = dynamic_cast<CWeaponPortalGun *>(pActiveWeapon);
	if ( pCannon )
	{
...
bool PortalPlayerPickupControllerIsHoldingEntity( CBaseEntity *pPickupControllerEntity, CBaseEntity *pHeldEntity )
{
	CPlayerPickupControllerPortalGun *pController = dynamic_cast<CPlayerPickupControllerPortalGun *>(pPickupControllerEntity);

	return pController ? pController->IsHoldingEntity( pHeldEntity ) : false;
}


float PortalGunGetHeldObjectMass( CBaseCombatWeapon *pActiveWeapon, IPhysicsObject *pHeldObject )
{
	float mass = 0.0f;
	CWeaponPortalGun *pCannon = dynamic_cast<CWeaponPortalGun *>(pActiveWeapon);
	if ( pCannon )
	{
		CGrabControllerPortalGun &grab = pCannon->GetGrabController();
...
CBaseEntity *PortalGetPlayerHeldEntity( CBasePlayer *pPlayer )
{
	CBaseEntity *pObject = NULL;
	CPlayerPickupControllerPortalGun *pPlayerPickupController = (CPlayerPickupControllerPortalGun *)(pPlayer->GetUseEntity());
...
{
	CWeaponPortalGun *pCannon = dynamic_cast<CWeaponPortalGun *>(pPhysCannon);
	if ( pCannon )
	{
...
float PortalPlayerPickupGetHeldObjectMass( CBaseEntity *pPickupControllerEntity, IPhysicsObject *pHeldObject )
{
	float mass = 0.0f;
	CPlayerPickupControllerPortalGun *pController = dynamic_cast<CPlayerPickupControllerPortalGun *>(pPickupControllerEntity);
	if ( pController )
	{
		CGrabControllerPortalGun &grab = pController->GetGrabController();
		mass = grab.GetSavedMass( pHeldObject );
	}
	return mass;
}
