...
class C_WeaponPortalGun: public C_BaseHLCombatWeapon
{
	DECLARE_CLASS( C_WeaponPortalGun, C_BaseHLCombatWeapon );
public:
...
STUB_WEAPON_CLASS_IMPLEMENT( weapon_portalgun, C_WeaponPortalGun );

IMPLEMENT_CLIENTCLASS_DT( C_WeaponPortalGun, DT_WeaponPortalGun, CWeaponPortalGun )
...
//-----------------------------------------------------------------------------
bool C_WeaponPortalGun::SetupEmitter( void )
{
...
//-----------------------------------------------------------------------------
void PortalComputeRenderInfo( mstudiobbox_t *pHitBox, const matrix3x4_t &hitboxToWorld, 
	...
	if ( SetupEmitter() )
	{
		...
		if ( m_bIsCurrentlyUpgrading )
		{
			...
			for ( i = 0; i < set->numhitboxes; ++i )
			{
				...
				PortalComputeRenderInfo( pBox, *hitboxbones[pBox->bone], &vecAbsOrigin, &xvec, &yvec );
